            ...
            {reviews.map((r) => (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="font-semibold text-white">{r.landlordName}</p>
                ...
                <p className="mt-3 text-sm text-gray-300">{r.reviewText}</p>
          ...
          <div className="space-y-4">
            {posts.map((p) => (
              ...
              >
                ...
                <p className="mt-2 line-clamp-2 text-sm text-gray-400">{p.content}</p>
                <p className="mt-2 text-xs text-gray-600">
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 font-medium text-white">@{r.landlordName}</p>
            </div>
            <Stars n={r.rating} />
          </div>
          <p className="mt-3 text-sm text-gray-300">{r.reviewText}</p>
  ...
  return (
    <span className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => (
        <Star
          key={i}
          className={`h-4 w-4 ${i <= Math.round(n) ? "fill-yellow-400 text-yellow-400" : "text-gray-600"}`}
        />
      ))}
    </span>
  );
